...
model_prefix = args.model_name if args.model_name else f'xgboost_bull_trend_{run_id}'
...
if 'regime' in df.columns:
    df = df[df['regime'] == 'TREND_BULL'].copy()
    print(f"Data difilter untuk Bull Trend: {len(df)} baris.")
else:
...
df['sl_buy_dist'] = np.maximum(df['sl_buy_dist'], 1.0 * df['atr_dollars']) # Minimum 1 ATR
...
df['sl_target'] = np.where(df['target'] == 1, df['sl_buy_dist'], np.where(df['target'] == -1, df['sl_sell_dist'], 0.0))
...
df['tp_target'] = np.where(df['target'] == 1, df['tp_buy_dist'], np.where(df['target'] == -1, df['tp_sell_dist'], 0.0))
...
# Convert to Binary Classification (1 = BUY, 0 = NEUTRAL/SELL)
y_cls = np.where(df['target'] == 1, 1, 0)
...
print(f"Class Distribution: {neg_count} NEUTRAL vs {pos_count} BUY (Weight: {scale_pos_weight:.2f})")
...
engine_dir = os.path.abspath(os.path.join(train_dir, "..", "quant-engine-v1", "ml_models", "bull_trend"))
kapsul_dir = os.path.abspath(os.path.join(train_dir, "mlflow", "models", "bull_trend"))
...
with PdfPages(pdf_path) as pdf:
    ...
    plt.axis('off')
    report_text = f"ENSEMBLE REPORT: BULL TREND\n"
    report_text += "="*40 + "\n"
    report_text += f"Classifier F0.5 : {f1_last:.4f}\n"
    report_text += f"Precision (BUY) : {report_dict.get('1', {}).get('precision', 0.0):.4f}\n"
    ...
    plt.title(f"Top 15 Features - Bull Trend (Classifier)")
    plt.tight_layout()
    pdf.savefig()
    plt.close()
    ...
    try:
        ...
        FeatureDriftTracker.save_feature_importance("TREND_BULL", feat_imp.to_dict())
    except ImportError:
        pass
...
try:
    mlflow.set_experiment("XAUUSD_H1_bull_trend")
except MlflowException:
    experiment = mlflow.get_experiment_by_name("XAUUSD_H1_bull_trend")
...
print(f"\nSUKSES! 3 Model ONNX untuk Bull Trend telah dibuat dan disimpan di direktori bull_trend.")
