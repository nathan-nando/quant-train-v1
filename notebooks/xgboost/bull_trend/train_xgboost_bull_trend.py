...
kapsul_dir = os.path.abspath(os.path.join(os.path.dirname(csv_path), "..", "model", "bull_trend"))
os.makedirs(engine_dir, exist_ok=True)
os.makedirs(kapsul_dir, exist_ok=True)

names = [
    ('xgboost_bull_trend.onnx', onnx_cls),
    ('xgboost_bull_trend_sl.onnx', onnx_sl),
    ('xgboost_bull_trend_tp.onnx', onnx_tp)
]
...
metadata = {
    "model_name": "xgboost_bull_trend",
    "features": list(X_all.columns),
    "classes": {0: "SELL", 1: "NEUTRAL", 2: "BUY"},
    "baseline_stats": stats
}
with open(os.path.join(engine_dir, 'xgboost_bull_trend_metadata.json'), 'w') as f:
    json.dump(metadata, f, indent=4)
with open(os.path.join(kapsul_dir, 'xgboost_bull_trend_metadata.json'), 'w') as f:
    json.dump(metadata, f, indent=4)

pdf_path = os.path.join(kapsul_dir, 'xgboost_bull_trend_report.pdf')
